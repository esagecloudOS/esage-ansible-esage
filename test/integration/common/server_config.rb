...
shared_examples 'server::config' do
  ...
  it 'has apache mappings to tomcat configured' do
    %w(api legal).each do |webapp|
      expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain("<Location /#{webapp}>")
      expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain("ProxyPass ajp://localhost:8010/#{webapp}")
      expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain("ProxyPassReverse ajp://localhost:8010/#{webapp}")
    end

    expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain('<Location /m>')
    expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain('ProxyPass http://localhost:8009/m')
    expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain('<Location /am2>')
    expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain('ProxyPass http://some_am:8009/am')
    expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain('Header add "Access-Control-Allow-Origin" "*"')
  end
end
