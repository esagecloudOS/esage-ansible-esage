...
describe 'Frontend configuration' do
  ...
  it 'has apache mappings to tomcat configured' do
    %w(api legal).each do |webapp|
      ...
      expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain("ProxyPass ajp://serverhost:8010/#{webapp}")
      expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain("ProxyPassReverse ajp://serverhost:8010/#{webapp}")
    end
    expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain('ProxyPass https://some_cms/blah')
    expect(file('/etc/httpd/conf.d/abiquo.conf')).to contain('ProxyPassReverse https://some_cms/blah')
